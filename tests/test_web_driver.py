...
def test_get_products_page_stats_aggregation(monkeypatch):
    class DummyPageWebDriver(DummyWebDriverShell):
        ...
        def get_products(self, _callback=None):
            ...
            items = ["$1.00\nItem A each", "$2.00\nItem B each"]
            if _callback:
                callback_result = _callback(items)
            return {'products': ['A', 'B'], 'incomplete_items': [{'name': 'A', 'missing': ['unit_price']}], 'page_stats': [{'page': 1, 'product_tiles': 2, 'scraped': 2, 'incomplete': 1}]}

    driver = DummyPageWebDriver()
    res = driver.get_products(lambda x: {'products': ['A', 'B'], 'incomplete_items': [{'name': 'A', 'missing': ['unit_price']}]} )
